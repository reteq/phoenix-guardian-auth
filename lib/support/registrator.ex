defmodule PhoenixTokenAuth.Registrator do
  ...
  def changeset(params = %{"account" => account}) when account != "" and account != nil do
    UserHelper.model.changeset(struct(UserHelper.model), params)
    |> Changeset.cast(params, ~w(account), [])
    |> Changeset.validate_change(:account, &Util.presence_validator/2)
    |> Changeset.unique_constraint(:account)
    |> changeset_helper
  end
  
  def changeset(user, params = %{"account" => account}) when account != "" and account != nil do
